...
#Our problem statement is concerned with mortality rates. To explore the relationship, we shall combine the Medicare dataset with another dataset that provides mortality rates. We shall then analyze correlations between Medicare spending per state and mortality rates to see if there is an association.
#We shall run a statistical analysis to determine if there’s a relationship between higher spending and lower mortality, or vice versa This will help in understanding whether states with lower public healthcare spending tend to have higher mortality rates.


#Import Process:
#We loaded the readr package: This package provides a fast and friendly way to read rectangular data like CSV files.

#We then specified the file path: The variable medicare_data_csv holds the path to the CSV file.

#We Read the CSV file: The read_csv function was used to import the data. We specified:

#col_types: Defining the data types for each column.
#col_names: Set to TRUE beacuse the first row contains column names.
#na: Definng which strings should be considered as missing values.
#View the data: Use head() to display the first few rows of the dataset to verify the import.
...
medicaid_data <- read_csv(
  ...
  col_types = cols(
    Location = col_character(), 
    `Seniors Spending` = col_double(),
    `Individuals with Disabilities Spending` = col_double(),
    `Adult Spending` = col_double(),
    `Children Spending` = col_double(),
    `Newly Eligible Adult Spending` = col_double(),
    `Total Spending` = col_double(),
...
#This dataset provides the number of deaths by age group, cause of death (ICD codes), and state. This will be the primary dataset used to assess mortality rates across different states, which is a key component of the problem statement. We shall calculate mortality rates for each state and compare them against the healthcare expenditure data.
#It breaks down deaths by state, allowing us to link the mortality information with the Medicaid or Medicare spending datasets from KFF. This linkage is necessary to explore the relationship between mortality rates and healthcare spending on a state-by-state basis. It also provides detailed breakdowns of deaths by age group and by specific causes (ICD codes). While this is not directly mentioned in the problem statement, this information could be used to provide deeper insights. For example, certain age groups or causes of death might show stronger correlations with healthcare spending (e.g., higher spending in a state might result in fewer deaths from preventable conditions).
#We shall identify states that stand out. For example, states with high mortality rates but low public healthcare spending could indicate underinvestment in healthcare.

#Import process
#Loaded Required Libraries:
#readr: For reading CSV files.
#dplyr: For data manipulation (though not used directly in this import).

#Read the CSV File:
#read_csv: Function used to import the CSV file.
#file: Specified the CSV file path stored in cdc_wonder_mortality_csv.
#Specified Column Types:
#col_types: Defined the data types for each column using cols().
#col_names = TRUE: Indicates that the first row contains column names.
#Handled Missing Values:na = c("", "NA", "NULL"): Specifies strings to be treated as missing values.
#head(): Displayed the first six rows of the imported data to verify the import.
...
#Mortality rates are typically calculated as the number of deaths per 1,000 or 100,000 individuals in a given population. To compute these rates for each state and age group, we need both the death counts (from the mortality dataset) and the population size (from this dataset). The population data by age group allows us to accurately adjust the mortality rates based on the size of each age cohort, making the rates more meaningful and comparable across states.
#Also, different states have varying age distributions, and older populations tend to have higher mortality rates. Using this population data, we can calculate age-adjusted mortality rates, which control for differences in the age structure between states. This allows for a fair comparison of mortality rates across states with different demographics.
#We shall also compute per capita healthcare spending (i.e., healthcare expenditures per person). This metric is crucial for assessing whether states with higher spending per person have better health outcomes (lower mortality rates). This population data helps normalize healthcare spending, enabling a more meaningful comparison between states, especially when looking at states with similar population sizes but different spending levels and health outcomes.
#Once the mortality rates are calculated using this population data, we shall explore whether states with higher populations in specific age groups (e.g., elderly) have higher healthcare spending and/or higher mortality rates. 

#Import process
#Read the CSV File:
#read_csv: Function used to import the CSV file.
#file: Specified the CSV file path stored in cdc_wonder_population_csv.
#Specified Column Types:
#col_types: Defined the data types for each column using cols().
#col_names = TRUE: Indicates that the first row contains column names.
#Handled Missing Values:na = c("", "NA", "NULL"): Specifies strings to be treated as missing values.
