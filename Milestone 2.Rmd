...
Import Process: #We loaded the readr package: This package provides a fast and friendly way to read rectangular data like CSV files. #We then specified the file path: The variable medicaid_data_csv holds the path to the CSV file. #We Read the CSV file: The read_csv function was used to import the data. We specified: #col_types: Defining the data types for each column. #col_names: Set to TRUE beacuse the first row contains column names. #na: Definng which strings should be considered as missing values. #View the data: Use head() to display the first few rows of the dataset to verify the import.
...
Import process #Read the CSV File: #read_csv: Function used to import the CSV file. #file: Specified the CSV file path stored in cdc_wonder_population_csv. #Specified Column Types: #col_types: Defined the data types for each column using cols(). #col_names = TRUE: Indicates that the first row contains column names. #Handled Missing Values:na = c("", "NA", "NULL"): Specifies strings to be treated as missing values. #head(): Displayed the first six rows of the imported data to verify the import.
...
The five data elements that are required for a specific scenario that is meant to investigate the association between healthcare spending and mortality rates is from the datasets.

Here are 5 examples of ones that we need:

Medicaid Dataset: Location/State Seniors Spending Adult Spending Total Spending Children Spending

CDC Wonder Population State Population Ten-Year Age Groups

CDC Wonder Mortality Residence Status Deaths Ten-Year Age Groups Cause of Death (UCD - ICD Sub-Chapter)
...
The Medicaid Dataset has Senior Spending, Adult Spending, Total Spending, and Children Spending all as character values. It would be better to convert these to either a numeric or a double value which we have done using the janitor function. This is because it will be easy to manipulate and perform summary statistics with this data if it is in that format vs. being a character.
